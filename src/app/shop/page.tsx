...
export default function ShopPage() {
    return (
        <div className="min-h-screen bg-black text-white font-sans selection:bg-neon-green selection:text-black overflow-x-hidden">
            {/* Navbar */}
            <nav className="fixed top-0 w-full z-50 flex items-center justify-between px-16 py-10 bg-black/80 backdrop-blur-sm">
            ...
            <main className="relative pt-32 pb-32 px-16 max-w-[1400px] mx-auto min-h-screen flex flex-col items-center justify-center overflow-visible">
                ...
                {/* Right Edge Technical Pattern (Vertical Bar) */}
                <div className="fixed right-6 top-1/2 -translate-y-1/2 flex flex-col gap-0.5 opacity-10 z-10 pointer-events-none">
                    {[...Array(24)].map((_, i) => (
                        <div key={i} className="w-2 h-[1px] bg-white"></div>
                    ))}
                    <div className="w-2 h-16 bg-white/20"></div>
                </div>

                {/* Bottom Technical Indicator pattern */}
                <div className="absolute bottom-16 right-16 flex items-center gap-4 opacity-5">
                    <div className="flex gap-0.5">
                        {[...Array(20)].map((_, i) => (
                            <div key={i} className="w-[1px] h-4 bg-white"></div>
                        ))}
                    </div>
                </div>
            </main>
            ...
            <footer className="px-12 pb-16 flex justify-between items-end">
                <div className="text-[9px] tracking-[0.4em] text-white/10 uppercase flex flex-col gap-2">
                    <span className="opacity-0 group-hover:opacity-100 transition-opacity">#X1001-LOG</span>
