...
const SITE_ID = 'ce7f39af-dff1-49ba-a74e-d49bddf9db06';
...
async function deploy() {
  ...
  if (deploy.required && deploy.required.length > 0) {
    ...
    for (const sha1 of deploy.required) {
      ...
      if (!filePath) {
        console.log('Warning: Could not find file for sha1:', sha1);
        continue;
      }
      ...
      if (uploadRes.status === 200) {
        ...
        console.log('\nError uploading', filePath, ':', uploadRes.status);
      }
    }
